    ...
    private = list(
        ...
        .pareto = function(image, ggtheme, theme, ...) {
            ...
            p <- ggplot2::ggplot(df, ggplot2::aes(x=x)) +
                ggplot2::geom_bar(ggplot2::aes(y=counts), width=0.6, fill=theme$fill[2], color=theme$color[2], stat="identity") +
                ggplot2::geom_point(ggplot2::aes(y=cum), size=3, color=theme$color[1]) +
                ggplot2::geom_path(ggplot2::aes(y=cum, group=1), size=1.1, lty="dashed", color=theme$color[1]) +
                ggplot2::scale_y_continuous(sec.axis = ggplot2::sec_axis(formula, name = "Cumalitive Percentage")) +
